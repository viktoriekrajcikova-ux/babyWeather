...
import { getOutfit, CLOTHES, type ClothesItem } from './clothesDeterminer'
...
describe('getOutfit', () => {
    ...
    it('kombinéza nahradí svrchní bundu a kalhoty', () => {
        const catalog : ClothesItem[] = [
            {name: 'snowsuit', imageUrl: 'assets/img/snowsuit.png', slot: 'wholeBody', tempFrom: null, tempTo: 15, ageFrom: 0, ageTo: null, sex: null, layer: 'outer'},
            {name: 'winter jacket', imageUrl: 'assets/img/snowsuit.png', slot: 'torso', tempFrom: null, tempTo: 15, ageFrom: 0, ageTo: null, sex: null, layer: 'outer'},
            {name: 'winter pants', imageUrl: 'assets/img/snowsuit.png', slot: 'legs', tempFrom: null, tempTo: 15, ageFrom: 0, ageTo: null, sex: null, layer: 'outer'}
        ]
    ...
    it('při 17 °C dítě dostane kompletní outfit bez bundy', () => {
        ...
        expect(names.sort()).toEqual([
            'hat', 'long shirt', 'shoes', 'sweater', 'warm sweatpants'
        ].sort())
        expect(result.filter(item => item.slot === 'legs').map(item => item.layer)).toEqual(['base'])
        expect(result.some(item => item.layer === 'outer')).toBe(false)
    })

    describe.each([null, 'male', 'female'] as const)('mírné počasí pro sex=%s', sex => {
        it.each([
            [15, 1], [15, 2], [15.01, 2], [16.99, 2], [17, 2], [19.99, 1], [19.99, 2]
        ...
        it('přesně při 20 °C přejde na letní outfit', () => {
            ...
            expect(names.sort()).toEqual([
                'shirt', sex === 'female' ? 'skirt' : 'shorts', 'hat', 'shoes'
            ].sort())
        })
    })
