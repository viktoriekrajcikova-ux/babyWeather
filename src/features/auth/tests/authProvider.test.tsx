...
import { childrenApi } from '../../children/api/childrenApi';
import { useChildrenQuery } from '../../children/hooks/useChildrenQuery';
import { useAddChildMutation } from '../../children/hooks/useAddChildMutation';
...
describe('AuthProvider se sdílenou cache dětí', () => {
  beforeEach(() => {
    ...
    vi.mocked(childrenApi.getChildren).mockReset().mockResolvedValue(childrenA);
    vi.mocked(childrenApi.addChild).mockReset();
    vi.mocked(childrenApi.deleteChild).mockReset();
  });
  ...
  it.each([
    ...
    async ({ logout, next, expected }) => {
      ...
      await waitFor(() => expect(childrenApi.getChildren).toHaveBeenCalledTimes(2));
  ...
  it.each([false, true])('opožděný fetch A nepřepíše B (přes odhlášení: %s)', async (logout) => {
    ...
    const { client } = setup();
    await waitFor(() => expect(childrenApi.getChildren).toHaveBeenCalledTimes(1));
    if (logout) {
      await emit('SIGNED_OUT', null);
      expect(screen.queryByTestId('children-list')).not.toBeInTheDocument();
      expect(client.getQueryState(key)).toBeUndefined();
  ...
  it.each([
    ...
    vi.mocked(childrenApi.deleteChild).mockReturnValueOnce(deletion.promise);
    vi.mocked(childrenApi.getChildren)
    ...
    await waitFor(() => {
      expect(childrenApi.deleteChild).toHaveBeenCalledWith(1);
      expectList([]);
    });
    ...
    if (next && expected) {
      vi.mocked(childrenApi.getChildren).mockResolvedValueOnce(expected);
      await emit('SIGNED_IN', next);
      await waitFor(() => expectList(expected));
    }
    const calls = vi.mocked(childrenApi.getChildren).mock.calls.length;
    await act(async () => {
      deletion.reject(new Error('delete failed'));
      await deletion.promise.catch(() => {});
    });
    await waitFor(() => expect(client.isMutating()).toBe(0));
    expect(childrenApi.getChildren).toHaveBeenCalledTimes(calls);
    if (expected) {
      expectList(expected);
      expect(client.getQueryData(key)).toEqual(expected);
  ...
  it.each([false, true])(
    ...
    async (switchToB) => {
      ...
      vi.mocked(childrenApi.deleteChild).mockResolvedValue(undefined);
      const { client } = setup();
      await waitFor(() => expectList(childrenA));
      ...
      try {
        ...
        expect(childrenApi.deleteChild).not.toHaveBeenCalled();
        expect(client.isMutating()).toBe(1);
        expect(client.getQueryData(key)).toEqual(childrenA);
        await emit('SIGNED_OUT', null);
        expect(client.getQueryState(key)).toBeUndefined();
        if (switchToB) {
          vi.mocked(childrenApi.getChildren).mockResolvedValue(childrenB);
          await emit('SIGNED_IN', sessionB);
          await waitFor(() => expectList(childrenB));
        }
        await act(async () => {
  ...
  it.each([
    ...
    vi.mocked(childrenApi.addChild).mockReturnValueOnce(addition.promise);
    vi.mocked(childrenApi.getChildren)
    ...
    expect(childrenApi.getChildren).toHaveBeenCalledTimes(calls + (next ? 0 : 1));
  ...
  it.each([false, true])(
    ...
    async (relogin) => {
      ...
      await waitFor(() => expect(app.client.isMutating()).toBe(0));
      expect(childrenApi.getChildren).toHaveBeenCalledTimes(relogin ? 1 : 2);
