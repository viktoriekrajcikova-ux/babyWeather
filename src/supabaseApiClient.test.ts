...
let supabaseApi: typeof import('./supabaseApiClient')['supabaseApi'];

beforeAll(async () => {
    vi.stubEnv('VITE_SUPABASE_URL', 'https://example.supabase.co');
    vi.stubEnv('VITE_SUPABASE_ANON_KEY', 'test-anon-key');
    ({ supabaseApi } = await import('./supabaseApiClient'));
});
...
describe('supabaseApi.getChildren', () => {
    beforeEach(() => {
        select.mockReset();
        from.mockClear();
    });
    ...
    it.each([
        ...
        await expect(supabaseApi.getChildren()).resolves.toEqual([
    ...
    it.each([{ data: null }, { data: [] }])('bez řádků vrátí prázdné pole ($data)', async ({ data }) => {
    ...
    it('předá chybu Supabase volajícímu', async () => {
        ...
        await expect(supabaseApi.getChildren()).rejects.toBe(error);
    });
});
