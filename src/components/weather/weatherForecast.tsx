...
const weatherForecast = ({ weatherForecastHourly, timezone, onClickForecast }: WeatherForecastProps) => {
    return <>
        ...
        <ul className={styles.list}>
            {weatherForecastHourly.map((weatherForecastHourlyItem, key) => <li key={key} onClick={() => onClickForecast(key)}>
                <div><WeatherIcon icon={weatherForecastHourlyItem.weather[0]?.icon ?? ''} description={weatherForecastHourlyItem.weather[0]?.description ?? ''} /></div>
                {Math.round(kelvinToCelsius(weatherForecastHourlyItem.temp))} °C
                {key === 0 ? <div>Now</div> : <div>{formatHour(weatherForecastHourlyItem.dt, timezone)}</div>}
            </li>)}
        </ul>
    </>
}
