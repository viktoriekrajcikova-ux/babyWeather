...
import geocoding from '../geocodingHandler';
const { getUser, getGeocoding, limit } = vi.hoisted(() => ({
  getUser: vi.fn(),
  getGeocoding: vi.fn(),
  limit: vi.fn(),
}));
vi.mock('../../lib/supabase', () => ({ supabaseServer: { auth: { getUser } } }));
vi.mock('../geocoding', () => ({ getGeocoding }));
vi.mock('../geocodingRateLimit', () => ({ geocodingRateLimit: { limit } }));
...
describe('geocoding endpoint', () => {
  ...
  it.each(['  Praha  ', 'České Budějovice', 'Ab', 'x'.repeat(100)])(
    'returns place for trimmed city %s after verifying and limiting the user',
    async (city) => {
      const response = await geocoding.fetch(
        request(new URLSearchParams({ city }).toString(), 'bEaReR   test-token'),
      );
      ...
      expect(limit.mock.invocationCallOrder[0]).toBeLessThan(
        getGeocoding.mock.invocationCallOrder[0],
      );
    },
  );
  ...
  it.each([
    ...
    'city=x',
    `city=${'x'.repeat(101)}`,
    'city=Praha&city=Brno',
    'city[]=Praha',
    'city=Praha&extra=value',
    'city=Praha%00',
    'city=%FF',
    'city=Praha%2',
    'city=Praha%0A',
  ...
  it.each([400, 401, 403, undefined, 0, 429, 500, 503])(
    'classifies auth error status %s',
    async (status) => {
      getUser.mockResolvedValueOnce({
        data: { user: null },
        error: { status, message: 'private detail' },
      });
      const unavailable = status === undefined || status === 0 || status === 429 || status >= 500;
      await expectError(
        await geocoding.fetch(request()),
        unavailable ? 503 : 401,
  ...
  it.each([null, '', 'Basic token', 'Bearer', 'Bearer token extra'])(
    'rejects invalid authorization %s',
    async (authorization) => {
      await expectError(
        await geocoding.fetch(request('city=Praha', authorization)),
        401,
        'Unauthorized',
      );
      expect(getUser).not.toHaveBeenCalled();
