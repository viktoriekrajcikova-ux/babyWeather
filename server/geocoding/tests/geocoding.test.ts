...
describe('getGeocoding', () => {
  ...
  it('caches only the mapped result for one day and reuses validated cached values', async () => {
    ...
    expect(redisSet).toHaveBeenCalledWith('babyweather:geocoding:open-meteo:v1:cs:Praha', place, {
      ex: 86400,
    });
  ...
  it.each([{ body: {} }, { body: { results: [] } }])(
    'returns null for no results without caching: $body',
    async ({ body }) => {
  ...
  it.each([
    ...
    fetchMock.mockResolvedValueOnce(
      Response.json({ results: [{ ...upstream.results[0], ...patch }] }),
    );
    await expect(getGeocoding('Praha')).rejects.toThrow();
    expect(redisSet).not.toHaveBeenCalled();
  });

  it.each([{ body: null }, { body: [] }, { body: { results: null } }, { body: { results: {} } }])(
