...
import weather from '../weatherHandler';
...
vi.mock('../weatherRateLimit', () => ({ weatherRateLimit: { limit } }));
...
const request = () =>
  new Request('https://example.test/api/weather?lat=50&lon=14', {
...
describe('weather endpoint with real Open-Meteo adapter and cache flow', () => {
  ...
  it('authenticates both requests but fetches provider data only on cache miss', async () => {
    ...
    expect(redisSet).toHaveBeenCalledWith('babyweather:weather:openmeteo:v2:50:14:en', expected, {
      ex: 300,
    });
  });
  ...
  it('invalid provider arrays become a generic 502 and are not cached', async () => {
    fetchMock.mockResolvedValueOnce(
      Response.json({ ...upstream, hourly: { ...upstream.hourly, apparent_temperature: [] } }),
    );
