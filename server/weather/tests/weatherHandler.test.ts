...
describe('weather – ověření požadavku', () => {
  ...
  it.each([
    ...
    {
      name: 'nepodporovaná metoda',
      method: 'POST',
      authorization: 'Bearer test-token',
      expectedStatus: 405,
    },
  ])(
    '$name nesmí spustit načítání počasí ani s platnými souřadnicemi',
    async ({ method, authorization, expectedStatus }) => {
      getUser.mockResolvedValueOnce({
        data: { user: null },
  ...
  it.each([400, 401, 403])(
    'při odmítnutí tokenu se statusem %s vrátí 401 bez interních podrobností',
    async (status) => {
      getUser.mockResolvedValueOnce({
        data: { user: null },
